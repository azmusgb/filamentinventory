(() => {
  ...
  function render() {
    ...
    host.innerHTML = `<section class="spool-action-summary" style="margin-top:18px"><span class="eyebrow">${esc(title)}</span><h3>${esc(s.id)} · ${esc(s.brand || 'Unknown')} ${esc(s.material || '')}</h3><p>${esc(s.colorName || '')}</p><p><strong>${esc(detail)}</strong></p>${row.loaded ? `<p class="muted">Already loaded: ${esc(s.printerName || 'Printer')} · ${esc(s.feederName || '')} ${esc(s.feederSlot || '')}</p>` : ''}<div class="dialog-actions"><button class="btn btn-primary" type="button" data-ready-action="${result.status === 'measurement-needed' ? 'weigh' : row.loaded ? 'open' : 'place'}" data-ready-id="${esc(s.id)}">${result.status === 'measurement-needed' ? 'Weigh now' : row.loaded ? 'Use this spool' : 'Load this spool'}</button></div></section>`;
  }
