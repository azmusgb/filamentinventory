...
export default async (req: Request) => {
  ...
  const legacyProfile = String(url.searchParams.get('profile') || '').trim();
  if (!/^[A-Za-z0-9._-]{1,32}$/.test(spool)) return response('Invalid spool ID.', 400, {'Content-Type':'text/plain; charset=utf-8'});
  if (legacyProfile && !['Bill','Aimee'].includes(legacyProfile)) return response('Invalid profile.', 400, {'Content-Type':'text/plain; charset=utf-8'});

  // Physical QR labels carry durable canonical spool identity only. A legacy
  // profile query is accepted for stale-client compatibility after validation,
  // but it is deliberately ignored and never encoded into the QR target.
