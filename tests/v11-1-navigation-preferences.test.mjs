...
test('V11 PWA registration and v38 cache publish the current shell while retaining the compatibility bridge offline', async () => {
  const [sw,pwa,version] = await Promise.all([read('sw.js'),read('pwa-client.js'),read('app-version.js')]);
  assert.match(sw, /const CACHE = 'filament-inventory-v38'/);
