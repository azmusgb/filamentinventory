...
test('action sheet adapts existing mutation surfaces instead of creating a second inventory engine', async () => {
...
test('action sheet progressively replaces card clutter while preserving native controls in the DOM', async () => {
  const client = await read('spool-actions-client.js');
