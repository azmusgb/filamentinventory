...
test('same-profile scan opens physical spool without navigation',async()=>{const scan=await read('scan-client.js'); assert.match(scan,/exists && resolved === current/); assert.match(scan,/openPhysicalSpool\(parsed\.spoolId\)/);});
