...
test('bulk mode provides explicit selection, visible targets and safe native batch actions', async () => {
  const client = await read('bulk-actions-client.js');
  for (const contract of ['data-bulk-start','data-bulk-visible','data-bulk-move','data-bulk-store','data-bulk-labels','data-bulk-archive','data-bulk-restore','bulkMoveDialog','bulkConfirmDialog','data-bulk-targets','data-confirm-targets']) {
  ...
  assert.match(client, /renderTargets\(dialog\.querySelector\('\[data-bulk-targets\]'\)\)/);
  assert.match(client, /renderTargets\(dialog\.querySelector\('\[data-confirm-targets\]'\)\)/);
...
test('bulk logical writes remain visible to the existing audit diff interceptor', async () => {
  ...
  assert.match(bulkClient, /localStorage\.setItem\(STORAGE_KEY,\s*JSON\.stringify\(nextState\)\)/);
...
test('bulk UI stays in the shared UI layer and supports mobile safe areas', async () => {
  const [legacyCss, v11Css] = await Promise.all([read('ui-system.css'),read('css/components/v11.css')]);
  assert.match(legacyCss, /\.fi-ui \.bulk-action-dock/);
  assert.match(legacyCss, /\.fi-ui \.bulk-select-control/);
  assert.match(legacyCss, /\.fi-ui \.bulk-move-dialog/);
  assert.match(v11Css, /fi-selected-target/);
  assert.match(v11Css, /env\(safe-area-inset-bottom\)/);
});

test('bulk QR labels reuse the existing label picker instead of a second QR engine', async () => {
  const client = await read('bulk-actions-client.js');
  for (const contract of ['labelSearch','data-label-id','labelPreviewGrid']) assert.ok(client.includes(contract));
  assert.match(client, /FilamentInventoryNavigation\?\.navigate\?\.\('labels'/);
...
test('bulk QR preparation re-queries the live picker after each rerender', async () => {
  const client = await read('bulk-actions-client.js');
  assert.match(client, /function selectLabelsSequentially\(ids,\s*index\s*=\s*0\)/);
