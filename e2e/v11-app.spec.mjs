...
const spool = (overrides = {}) => ({
  ...
  lastDriedDate: '',
  notes: 'Deterministic browser-test spool.',
  ...
  const payload = {
    ...
    billState: state('Bill', [
      ...
      spool({
        ...
        visualPercent: 10,
        location: 'Rack B',
        notes: 'Low PETG browser-test spool.',
      }),
    ]),
    aimeeState: state('Aimee', [
      spool({
        id: 'A001',
        owner: 'Aimee',
        material: 'PLA',
...
async function navigate(page, view) {
  await page.evaluate(target => {
    globalThis.FilamentInventoryNavigation.navigate(target, { historyMode: 'push', focus: false });
  }, view);
...
test('boots with one active surface and closed dialogs never render in layout', async ({ page }) => {
  ...
  const renderedClosedDialogs = await page.locator('dialog:not([open])').evaluateAll(nodes =>
...
test('preference edits autosave and flush before leaving Preferences', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => {
    const saved = JSON.parse(localStorage.getItem('filament-user-v1:bill:preferences') || '{}');
    return saved.identity?.initials;
  })).toBe('BW');
});
...
test('mobile Home and Inventory preserve approved visual hierarchy', async ({ page }, testInfo) => {
  ...
  await navigate(page, 'inventory');
  await expect(page).toHaveScreenshot('mobile-inventory.png', { fullPage: true });
});
