(() => {
  ...
  function measurement(spool) {
    ...
    if (validNum(spool?.gross) && validNum(spool?.tare) && Number(spool.gross) >= Number(spool.tare)) {
      ...
      const percent = start ? Math.round((grams / start) * 1000) / 10 : null;
    ...
    if (validNum(spool?.visualPercent)) {
      ...
      if (!start) return {grams:null, percent, source:'Visual estimate'};
  ...
  function remainingLabel(m) {
    if (m.grams !== null && m.percent !== null) return `${Math.round(m.grams)} g · ${Math.round(m.percent)}%`;
    if (m.grams !== null) return `${Math.round(m.grams)} g · ${m.source}`;
    if (m.percent !== null) return `Grams unknown · ${Math.round(m.percent)}% visual`;
  ...
  function renderPicker() {
    ...
    list.innerHTML = rows.length ? rows.map(spool => {
      const m = measurement(spool);
      const status = spool.archivedAt ? 'Archived' : (m.grams === null ? (m.percent === null ? 'Unknown remaining' : `${Math.round(m.percent)}% visual · grams unknown`) : `${Math.round(m.grams)} g`);
  ...
  function showScanDialog(id) {
    ...
    const remain = remainingLabel(m);
    const swatch = /^#[0-9a-f]{6}$/i.test(spool.colorHex || '') ? spool.colorHex : '#64748b';
    body.innerHTML = `<div class="scan-summary"><i class="scan-swatch" style="background:${swatch}"></i><div><strong>${esc(spool.id)}</strong><span>${esc(spool.brand || 'Unknown')} · ${esc(spool.material || 'Unknown')} · ${esc(spool.colorName || 'Unknown')}</span><span>${esc(remain)} · ${esc(m.source)}</span><span>${esc(spool.location || 'Location not set')}${spool.archivedAt ? ' · Archived' : ''}</span></div></div>`;
