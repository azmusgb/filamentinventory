(() => {
  ...
  function labelMarkup(spool, preview = true) {
    ...
    return `<article class="physical-print-label"><img alt="" src="${qr}"/><div><strong>${esc(spool.id)}</strong><span>${esc(spool.brand || 'Unknown')} · ${esc(spool.material || 'Unknown')}</span><span>${esc(spool.colorName || 'Unknown')}</span><span>${esc(remain)}${spool.location ? ` · ${esc(spool.location)}` : ''}</span><small>Scan to open / weigh</small></div></article>`;
  }
  ...
  function init() {
    ...
    if (pendingSpoolId && pendingScan) {
      const clean = new URL(location.href); clean.searchParams.delete('scan');
      history.replaceState(null, '', clean.pathname + clean.search + clean.hash);
      setTimeout(() => showScanDialog(pendingSpoolId), 420);
    }
  }
