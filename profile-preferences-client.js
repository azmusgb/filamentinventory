(() => {
  ...
  function openPreferences(){
    document.querySelectorAll('.view').forEach(el=>el.classList.toggle('active',el.id==='preferencesView'));
    document.querySelectorAll('.tab[data-view]').forEach(el=>el.setAttribute('aria-selected',String(el.dataset.view==='preferences')));
    history.replaceState(null,'',`${location.pathname}#view=preferences`);
  ...
  function ensureSurface(){
    ...
    if(!tab){
      ...
      tab.addEventListener('click',openPreferences);
    }
  ...
  function save(event){
    ...
    apply();render();globalThis.FilamentInventoryEvents?.emit?.('profile:preferences-changed',{owner:owner()});
  }
  ...
  function apply(){
    ...
    const reorder=$('reorderThreshold'); const start=$('startWeight'); const adding=$('dialogTitle')?.textContent?.toLowerCase().includes('add');
  ...
  function init(){ensureSurface();render();apply();applyStartView();new MutationObserver(()=>apply()).observe(document.body,{childList:true,subtree:true});}
